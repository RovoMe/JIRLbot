...
public class RobotsCacheDispatcher extends NullDispatcher<HostData, StringSerializer>
{
	...
	private final static Logger logger = LogManager.getLogger(RobotsCacheDispatcher.class);
	 ...
	 * are returning a duplicate key. Check the returned host data if the URL is
	...
	/**
	 * <p>
	 * No robots.txt file could be found for this URL yet
	 * </p>
	 */
	@Override
	public void uniqueKeyCheck(Long key, StringSerializer url) 
	{
		logger.debug("No robots.txt found for {} inside DRUM!", url);
		for (RobotsCachePassedListener listener : this.listeners)
	 ...
	 * @param crawlDelay
	 *            A {@link String} that will hold the delay between to page
