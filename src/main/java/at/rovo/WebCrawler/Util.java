...
public class Util
{
	 ...
	 * @return The pay level domain of the provided URL or null if the pay level
	...
	public static String getPLDofURL(String url)
	{
		...
		try
		{
			...
			if (rest.endsWith(".co") || rest.endsWith("wa.edu")
					|| rest.endsWith(".edu") || rest.endsWith(".ac")
					|| rest.endsWith(".go"))
			{
	 ...
	 * </p>
	 * <p>
	 * F.e the directory path of <em> 
	 * http://http://irl.cs.tamu.edu/people/hsin-tsang/papers/www2008.pdf</em>
	 * is <em>hsing-tsang/papers/www2008.pdf</em>.
	 * </p>
	 * 
	 * @param url
			...
			throws IOException, DrumException
	{
		...
		do
		{
			...
			if (data != null)
			{
				...
				if (hostData != null && logger.isInfoEnabled())
					logger.info("Key: " + data.getFirst() + ", Value: "
							+ hostData);
	...
	/**
	 * <p>
	 * Checks if a URL is in an absolute format and if not it tries to transform
	 * it into an absolute URL using the parent URL.
	 * </p>
	 * <p>
	 * Note that URLs that start with an anchor, javascript or mailto tag are
	 * discarded and null will be returned therefore.
	 * </p>
	 * 
	 * @param url
	 *            The URL that should be checked for its absolute format
	 * @param parentURL
	 *            The parent URL of <em>url</em> which will be used to transform
	 *            the relative URL into an absolute one
	...
	public static String checkAndTransformURL(String url, String parentURL)
	{
		...
		else if (url.startsWith("/"))
		{
			...
			String parent = parentURL.substring(0,
			...
			if (parent.endsWith("/"))
			{
				logger.debug("url starts with '/': " + parentURL + " | "
						+ parent + " | " + url.substring(1));
		...
		else if (url.startsWith(".."))
		{
			...
			if (parentURL.endsWith("/")
					&& ("http:/".equals(parentURL.subSequence(0, parentURL
							.lastIndexOf("/"))) || "https:/".equals(parentURL
							.subSequence(0, parentURL.lastIndexOf("/")))))
				numOfDirsUp = 0;
			...
			if (parent.equals("http:/") || parent.equals("https:/")
					|| parent.equals("http:") || parent.equals("https:")) 
					// && numOfDirsUp == 1)
				parent = parentURL;
			...
			logger.debug("url starts with '..': " + parentURL + " | " + parent
					+ " | " + url + " | " + tmp + " | " + ret);
		...
		else if (!url.startsWith("http://") && !url.startsWith("https://"))
		{
			...
			{
				if (logger.isDebugEnabled())
					logger.debug("url starts not with 'http://': " + parentURL
							+ " | " + parent + " | " + url);
				return parent + url.substring(1);
			}
