	...
	JPanel implements ActionListener, TreeSelectionListener
{
	...
	private void readBucketFiles() throws Exception
	{
		...
		{
			this.bucketTree.add(new DefaultMutableTreeNode("Bucket "+i+" - "+kvFiles[i].length()+" bytes / "+auxFiles[i].length()+" bytes"));
		}
	}
		...
		try
		{
			...
			while (rkvFile.getFilePointer() < rkvFile.length())
			{
				...
				if ('c' == op)
					...
					buffer.append("UNDEFINED SYMBOL: "+op);
				...
				if (rkvFile.getFilePointer()+8 < rkvFile.length())
				{
					...
					if (valueSize > 0)
					{
						...
						V value = DrumUtil.deserialize(valueBytes, this.valueClass);
						buffer.append(" - ");
						buffer.append("Value: ");
						buffer.append(value);
					}
				...
				if (rauxFile.getFilePointer()+4 < rauxFile.length())
				{
					...
					if (auxSize > 0)
					{
						...
						A aux = DrumUtil.deserialize(auxBytes, this.auxClass);
						buffer.append(" - ");
						buffer.append("Aux: ");
						buffer.append(aux);
					}
